...
export function ModelDataTab({ model }: ModelDataTabProps) {
  ...
  const handleViewDetails = (layer: any) => {
    const layerDetails: Layer = {
      ...
      memoryUsage:
        (layer.weight_tensor?.shape?.reduce((a: number, b: number) => a * b, 0) || 0) * 4, // 4 bytes per value (assuming float32)
  ...
  const layers = model.graphs && model.graphs.length > 0 ? model.graphs[0].layers || [] : [];
  ...
  const filteredLayers = layers.filter((layer: any, idx: number) => {
    ...
    return (
      layerName.toLowerCase().includes(searchQuery.toLowerCase()) ||
        ...
        {model.graphs && model.graphs.length > 0 ? (
                ...
                >
                  <Grid columns="2" gap="5">
                    <Box>
                      <Text
                        size="1"
                        style={{ color: "var(--gray-11)", marginBottom: "6px", marginRight: "6px" }}
                      >
                        Model Name
                      </Text>
            ...
            >
              <Box style={{ marginTop: "28px" }}>
                ...
                >
                  ...
                  >
                    This visualization shows how dimensions change across layers. Follow the flow of
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <p style={{ margin: "4px 0 0 0" }}>
                                    ...
                                    {payload[0].payload.inputDim}
                                  </p>
                                  <p style={{ margin: "4px 0 0 0" }}>
                                    <span style={{ color: "#FF8C66", fontWeight: 600 }}>
                                      Output Dim:
                                    </span>{" "}
                                    {payload[0].payload.outputDim}
                                  </p>
                                  <p style={{ margin: "8px 0 0 0" }}>
                                    <span style={{ fontWeight: 600 }}>Weight Shape:</span>{" "}
                                    {payload[0].payload.weightShape}
                                  </p>
                                  <p style={{ margin: "4px 0 0 0" }}>
                                    <span style={{ fontWeight: 600 }}>Has Weights:</span>{" "}
                                    {payload[0].payload.hasWeights ? "Yes" : "No"}
                                  </p>
                                  <p style={{ margin: "4px 0 0 0" }}>
                                    <span style={{ fontWeight: 600 }}>Has Bias:</span>{" "}
                                    {payload[0].payload.hasBias ? "Yes" : "No"}
                                  </p>
                                </div>
                              );
                            }
                          ...
                          >
                            ...
                            <Flex direction="column" align="center" gap="2">
                              ...
                              {layer.weight_tensor?.shape && (
                                <Text
                                  size="1"
                                  style={{ marginTop: "8px", color: "#666", textAlign: "center" }}
                                >
                          ...
                          >
                            ...
                            <Table.Cell>
                              <Text style={{ fontFamily: "monospace", color: "#FF5733" }}>
                                {layer.weight_tensor?.shape
                                  ? layer.weight_tensor.shape.join(" × ")
                                  : "-"}
                              </Text>
                            </Table.Cell>
                            <Table.Cell>
          ...
          >
            ...
            >
              ...
              <Text size="5" style={{ fontWeight: 600, color: "#FF5733" }}>
                No Data Available
              </Text>
              <Text
                style={{
              ...
              >
                No data could be found for this model. The model may not be fully deployed on-chain
                yet.
              </Text>
              <Badge variant="soft" color="orange" size="2" style={{ marginTop: "12px" }}>
                No Model Data
              </Badge>
            </Flex>
          </motion.div>
              ...
              <Flex direction="column" gap="4">
                ...
                >
                  ...
                  <Text size="2" style={{ color: "#666", marginBottom: "8px" }}>
                    Type:{" "}
                    <Badge variant="soft" color="orange" style={{ marginLeft: "4px" }}>
                      {selectedLayer.type}
                    </Badge>
                  </Text>
                </Card>
                    ...
                    <Card style={{ padding: "16px" }}>
                      ...
                      <Text as="div" size="3" style={{ fontFamily: "monospace", color: "#FF5733" }}>
                        {selectedLayer.weights
                          ? selectedLayer.weights.flat().length.toLocaleString()
                          : "0"}
                      </Text>
                    </Card>
                ...
                {selectedLayer.weights && (
                  <Box>
                    ...
                    >
                      ...
                      >
                        {formatMatrix(
                          selectedLayer.weights.slice(0, 5).map(row => row.slice(0, 5))
                        )}
