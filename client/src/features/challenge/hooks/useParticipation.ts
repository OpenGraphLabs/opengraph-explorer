...
// Simulate API delay
const simulateApiDelay = (ms: number = 600) => 
...
export function useParticipation(userId: string) {
  ...
  const getParticipationStats = () => {
    ...
    const totalAnnotations = participations.reduce((total, p) => 
      total + p.progress.label.completed + p.progress.bbox.completed + p.progress.segmentation.completed, 0
    );
