...
export const PHASE_CONSTRAINTS: Record<ChallengePhase, PhaseConstraints> = {
  label: {
    ...
    description: 'In the label phase, you can only add text labels to identify entities in the image. Bounding box and segmentation tools will be available in later phases.'
  },
  bbox: {
    allowedTools: ['label', 'bbox'],
    message: 'BBox Phase: Label and bounding box annotations are allowed',
    description: 'In the bounding box phase, you can add labels and draw bounding boxes around entities. Segmentation tools will be available in the next phase.'
  },
  segmentation: {
    allowedTools: ['label', 'bbox', 'segmentation'],
    message: 'Segmentation Phase: All annotation tools are available',
    description: 'In the segmentation phase, you can use all annotation tools: labels, bounding boxes, and precise polygon segmentation.'
  },
  validation: {
    allowedTools: [],
    message: 'Validation Phase: Annotation is disabled during validation',
    description: 'The challenge is currently in the validation phase. New annotations cannot be added while validators review submissions.'
  },
  completed: {
    allowedTools: [],
...
export function getDisallowedToolMessage(phase: ChallengePhase, tool: AnnotationType): string {
  ...
  switch (phase) {
    case 'label':
      ...
      if (tool === 'segmentation') return 'Segmentation annotation will be available in the Segmentation phase';
      break;
    case 'bbox':
      if (tool === 'segmentation') return 'Segmentation annotation will be available in the Segmentation phase';
      break;
...
export function getNextAvailablePhase(currentPhase: ChallengePhase, desiredTool: AnnotationType): ChallengePhase | null {
