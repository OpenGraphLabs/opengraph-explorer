  ...
  const addLabel = useCallback(
    (label: string) => {
      ...
      if (stackState.isFull) {
        console.warn(
          `Annotation stack is full (${maxSize}/${maxSize}). Please save annotations before adding more.`
        );
        return false;
      }
    ...
    (index: number) => {
      if (images[index]) {
        ...
        dispatch({
          ...
          payload: { ...state, currentImageIndex: index } as any,
        });
      }
    },
    [images, state]
