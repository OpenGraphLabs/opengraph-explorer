...
export function useBBoxInteraction(
  ...
  const throttledUpdate = useCallback((id: string, updates: Partial<BoundingBox>) => {
  ...
  const updateBBoxInteraction = useCallback((currentPoint: Point) => {
    if (isDraggingBbox && activeHandle && originalBbox) {
      const updates = updateBBoxFromHandle(originalBbox, activeHandle, currentPoint, dragStartPoint);
      throttledUpdate(originalBbox.id, updates);
    }
  }, [isDraggingBbox, activeHandle, originalBbox, dragStartPoint, updateBBoxFromHandle, throttledUpdate]);
