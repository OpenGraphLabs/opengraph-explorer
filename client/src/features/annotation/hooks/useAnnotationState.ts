...
export function useAnnotationState() {
  ...
  const addAnnotation = useCallback((item: DataObject, index: number, value: string) => {
    ...
    setAnnotationState(prev => {
      const existingIndex = prev.pendingAnnotations.findIndex(annotation => annotation.path === item.path);
      const now = Date.now();
      ...
      if (existingIndex >= 0) {
        ...
        return {
          ...prev,
          pendingAnnotations: [...prev.pendingAnnotations, { 
            path: item.path, 
            label: [value],
            timestamp: now
          }],
  ...
  const savePendingAnnotations = useCallback(async (selectedDataset: DatasetObject) => {
    ...
    try {
      ...
      const result = await datasetSuiService.addAnnotationLabels(selectedDataset, annotationState.pendingAnnotations);
