...
export function useAnnotationStack() {
  ...
  const addToStack = useCallback((
    ...
    if (stack.length >= MAX_STACK_SIZE) {
      console.warn('Annotation stack is full. Please save current annotations before adding more.');
      return false;
    }
  ...
  const prepareForSuiSave = useCallback((): DataAnnotationInput[] => {
    ...
    const groupedByImage = stack.reduce((acc, item) => {
    ...
    // Sui 저장용 형식으로 변환
    return Object.values(groupedByImage).map(group => ({
   ...
   */
  const findInStack = useCallback((imageId: string, annotationType: 'label' | 'bbox', annotationId: string) => {
