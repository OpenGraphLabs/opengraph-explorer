...
} from "@/shared/api/sui/annotationSuiService";
...
export function useAnnotationSave() {
  ...
  const saveAnnotations = useCallback(
    async (datasetId: string, dataAnnotations: DataAnnotationInput[]) => {
      ...
      try {
        ...
        console.log(
          `Saving ${totalAnnotations} annotations across ${dataAnnotations.length} data items...`
        );
  ...
  const canSave = useCallback(
    (dataAnnotations: DataAnnotationInput[]) => {
      ...
      const hasValidAnnotations = dataAnnotations.some(
        data =>
      ...
      // dataId 유효성 확인
      const hasInvalidDataId = dataAnnotations.some(
        data => !data.dataId || data.dataId.trim() === ""
      );
      if (hasInvalidDataId) {
